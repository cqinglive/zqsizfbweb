    ...
    <div v-if="gerenjiben">
      <div v-if="empty == false" class="weui-cells" style="font-size:12px">
        <div class="weui-cell">
          ...
          <div class="weui-cell__ft">{{Account.user_name}}</div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__bd">
            <p>证件号码</p>
          </div>
          <div class="weui-cell__ft">{{Account.cert_no}}</div>
        </div>
      </div>
    ...
    <div v-if="cxyb">
      ...
      >
        <div class="weui-cell" style="background-color:#FFFFF0">
          ...
          <div class="weui-cell__ft">{{itemdata.AAE001}}</div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__bd">
            <p>金额</p>
        ...
        <div class="weui-cell">
          ...
          <div class="weui-cell__ft">{{itemdata.AAB301_NAME}}</div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__bd">
            <p>缴费方式</p>
          </div>
          <div class="weui-cell__ft">{{itemdata.BAE181}}</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="ybgz">
      ...
      >
        <div class="weui-cell" style="background-color:#FFFFF0">
          ...
          <div class="weui-cell__ft">{{itemdata.AAE002}}</div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__bd">
            <p>待遇金额</p>
          </div>
          <div class="weui-cell__ft">{{itemdata.HBJE}}元</div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__bd">
            <p>发放方式</p>
          </div>
          <div class="weui-cell__ft">{{itemdata.HBFS_NAME}}</div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__bd">
            <p>所属经办机构</p>
          </div>
          <div class="weui-cell__ft">{{itemdata.AAB034_NAME}}</div>
        </div>
      </div>
    </div>
...
export default {
    ...
    conditionsearch(y) {
      ...
      switch (flag) {
        case "TYWT_GRJBXX_GGCX_01":
          ...
          url = `/web/socialzfb/${flag}?username=${common.Encrypt(
            ot.Account.user_name
          )}&idcard=${common.Encrypt(ot.Account.cert_no)}`;
          break;
        case "TYWT_ZGSB_GGCX_01":
          ...
          url = `/web/socialzfb/${flag}?username=${common.Encrypt(
            ot.Account.user_name
          )}&idcard=${common.Encrypt(ot.Account.cert_no)}&year=${
            ot.CurrentYear
          }`;
          break;
        ...
        case "07":
          ...
          url = `/web/social30zfb/MAIN_PROC_YL_MONTH/${
            ot.Account.socialid
          }/${common.Encrypt(ot.Account.cert_no)}/${ot.CurrentYear}`;
          break;
        case "12":
          //城乡医保
          ot.cxyb = true;
          ot.Tip = "年城乡医保信息查询 业务查询结果：";
          url = `/web/social30zfb/GX_05/${ot.Account.socialid}/${common.Encrypt(
            ot.Account.cert_no
          )}/${ot.CurrentYear}`;
          break;
        case "11":
          ...
          url = `/web/oldsocial/query/${ot.Account.socialid}/${common.Encrypt(
            ot.Account.cert_no
          )}/${flag}/${ot.CurrentYear}`;
          break;
        case "10":
          ...
          url = `/web/social30zfb/MAIN_PROC_SY_MONTH/${
            ot.Account.socialid
          }/${common.Encrypt(ot.Account.cert_no)}/${ot.CurrentYear}`;
          break;
        case "19":
      ...
      ot.$axios
        ...
        .then(res => {
          ...
          ot.responsedata.forEach(item => {
            if (item.AAE044) {
              item.AAE044 = common.Decrypt(item.AAE044);
            }
          });
          console.log(res);
        })
  ...
  mounted() {
    ...
    let flag = this.$store.getters.getFlag;
    ot.responsedata = [];
    ot.Account = ot.$store.getters.getUserinfo;
    ...
    switch (flag) {
      ...
      case "TYWT_ZGSB_GGCX_01":
        ...
        url = `/web/socialzfb/${flag}?username=${common.Encrypt(
          ot.Account.user_name
        )}&idcard=${common.Encrypt(ot.Account.cert_no)}&year=${ot.CurrentYear}`;
        break;
      case "TYWT_ZGSB_GGCX_02":
        //失业缴费信息查询
        ot.shiye = true;
      ...
      case "19":
        ...
        ot.ybgz = true;
        ot.Tip = "年医保个账信息查询 业务查询结果：";
        url = `/web/social30zfb/YIL_03/${ot.Account.socialid}/${common.Encrypt(
          ot.Account.cert_no
        )}/${ot.CurrentYear}`;
        break;
      case "TYWT_RYCBXX_01":
